...
EXPECTED_REASON_REFINEMENTS = {
    "WS:SRC:ZA_TOP40:2DF0FDC7A9EB6AF3",  # AB InBev
    "WS:SRC:ZA_TOP40:3598A879F2EA6E99",  # Sanlam
    "WS:SRC:ZA_TOP40:5F247CB49B9B155E",  # Vodacom
}
...
def requalify() -> dict[str, Any]:
    ...
    try:
        ...
        reason_refinements = changed_df.loc[
            changed_df["WS_ID"].isin(EXPECTED_REASON_REFINEMENTS)
            & changed_df["Status_Before"].eq("QUARANTINE")
            & changed_df["Reason_Before"].eq("INVALID_OHLC_OR_VOLUME")
            & changed_df["Status_After"].eq("QUARANTINE")
            & changed_df["Reason_After"].eq("SUSPICIOUS_RETURN_NEEDS_REPAIR")
            & changed_df["Suspicious_Returns"].gt(0)
        ].copy()

        if set(reason_refinements["WS_ID"].astype(str)) != EXPECTED_REASON_REFINEMENTS:
            raise SystemExit(
                "Expected exactly the three documented quarantine reason refinements. "
                + json.dumps(
                    {
                        "expected": sorted(EXPECTED_REASON_REFINEMENTS),
                        "got": sorted(reason_refinements["WS_ID"].astype(str).tolist()),
                    },
                    ensure_ascii=False,
                )
            )
        ...
        unexpected_changes = changed_df.loc[
            ...
            & ~changed_df["WS_ID"].isin(reason_refinements["WS_ID"])
    ...
    reason_refinements.to_csv(OUTPUT_DIR / "qa_reason_refinements_v0.4.csv", index=False)
    ...
    return {
        ...
        "reason_refinements": len(reason_refinements),
...
def main() -> int:
    ...
    if args.self_test:
        ...
        assert len(EXPECTED_REASON_REFINEMENTS) == 3
    ...
    payload["notes"] = [
        ...
        "AB InBev, Sanlam and Vodacom remain QUARANTINE; only their reason code is refined from INVALID_OHLC_OR_VOLUME to SUSPICIOUS_RETURN_NEEDS_REPAIR.",
